...
export default function WorkspaceClient({
  ...
  // Auto-show new session dialog for first-time users (always for demo)
  useEffect(() => {
    const graphs = graphsQuery.data?.items ?? [];
    if (graphsQuery.isLoading || graphs.length > 0) return;
    ...
    if (!hasSeenEmptyState) {
      setAutoShowNewSession(true);
      localStorage.setItem("hasSeenEmptyState", "true");
    }
  }, [graphsQuery.data, graphsQuery.isLoading, isDemo]);
